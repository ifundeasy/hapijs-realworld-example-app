...
module.exports = (server) => {
  return {
    ...
    async getArticles (request, h) {
      const user = request.auth.isAuthenticated ? request.auth.credentials.user : null
    ...
    getArticle (request, h) {
      let article = request.pre.article.toJSONFor(null)
    ...
    async deleteArticle (request, h) {
      try {
        await server.methods.services.articles.delete(request.pre.article)
